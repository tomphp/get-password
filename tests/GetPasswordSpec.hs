  ...
  describe "getPasswords" $ do
    ...
    it "errors when show password fails" $ do
      let (result, history) = runMock $ do
            listPasswordsWillReturn
              ( Right
                  [ PasswordEntry {entryId = "entry-id", name = "contains search", url = "url"}
                  ]
              )
    ...
    it "errors when no matches are found" $ do
      let (result, history) = runMock $ do
            listPasswordsWillReturn
              ( Right
                  [ PasswordEntry {entryId = "entry-id", name = "does-not-match", url = "url"}
                  ]
              )
            showPasswordWillReturn (Left $ LastPassShowPasswordFailed "reason")
            getPassword "search"
    ...
    it "errors when multiple matches are found" $ do
      let (result, history) = runMock $ do
            listPasswordsWillReturn
              ( Right
                  [ PasswordEntry {entryId = "entry-id-1", name = "match one", url = "url1"},
                    PasswordEntry {entryId = "entry-id-2", name = "match two", url = "url2"}
                  ]
              )
            showPasswordWillReturn (Left $ LastPassShowPasswordFailed "reason")
            getPassword "match"
    ...
    it "returns the password matching the name" $
      do
        let (result, history) = runMock $ do
              ...
              listPasswordsWillReturn
                ( Right
                    [ PasswordEntry {entryId = "entry-id-1", name = "contains search", url = "url1"},
                      PasswordEntry {entryId = "entry-id-2", name = "other", url = "url2"}
                    ]
                )
              getPassword "search"
    ...
    it "returns the password matching the url" $ do
      let (result, history) = runMock $ do
            ...
            listPasswordsWillReturn
              ( Right
                  [ PasswordEntry {entryId = "entry-id-1", name = "does-not-match", url = "url1"},
                    PasswordEntry {entryId = "entry-id-2", name = "matches", url = "http://example.com"}
                  ]
              )
            getPassword "example"
      result `shouldBe` Right "secret"
